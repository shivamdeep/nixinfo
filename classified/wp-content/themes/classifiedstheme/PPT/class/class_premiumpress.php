	...
	function Articles($num=10,$details=false,$date=false){
		...
		foreach($posts as $post){
			...
			if($details){
				...
				$STRING  .="<li><a class=\"cus_art\" href='".get_permalink( $post->ID )."'>".$post->post_title."</a><img src='".$this->Image($post,"url")."'/>".$cont."</li>";			
			 
			}else{ 
			
			
				$STRING  .="<li><a class=\"cus_art\" href='".get_permalink( $post->ID )."'>".$post->post_title."</a></li>";			
...
/************ SHRINK THE WEB FUNCTIONALITY ******************/
	...
	function CheckExpired($post_id,$POSTDATE=""){ 
		...
		if(is_numeric($post_expires)){
			...
			if( date('Y-m-d h:i:s') > date('Y-m-d h:i:s',strtotime(date("Y-m-d h:i:s", strtotime($POSTDATE )) . " +".$post_expires." days")) ){	 			
